        ...
        while self.is_running:
            try:
                ...
                while action_index < len(parsed_actions) and self.is_running:
                    ...
                    # After a successful action, check how much the screen changed.
                    # MAJOR change (>15% pixels) = likely a phase transition, popup,
                    # or significant UI shift → discard remaining plan, get a fresh
                    # LLM analysis on the next loop iteration.
                    # MINOR change = coordinates may have shifted → revalidate next
                    # action's coordinates with vision matching.
                    ...
                    if action_succeeded and after_pil is not None and action_index < len(parsed_actions) - 1:
                        ...
                        if major_change:
                            ...
                            action_index += 1
                            break  # exit action loop → main loop takes fresh screenshot + LLM call
